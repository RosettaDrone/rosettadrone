...
static void gst_native_init(JNIEnv *env, jobject thiz, jstring ip, jint port, jint bitrate, jint encode_speed) {
    CustomData *data = g_new0 (CustomData, 1);
    SET_CUSTOM_DATA (env, thiz, custom_data_field_id, data);
    GST_DEBUG_CATEGORY_INIT (debug_category, "video_encoder", 0, "GST Video Encoding");
    gst_debug_set_threshold_for_name("video_encoder", GST_LEVEL_DEBUG);
    GST_DEBUG ("Created CustomData at %p", data);
    data->app = (*env)->NewGlobalRef(env, thiz);
    GST_DEBUG ("Created GlobalRef for app object at %p", data->app);

    const char *desired_ip = (*env)->GetStringUTFChars(env, ip, NULL);
    data->ip = (char *) desired_ip;
    GST_DEBUG ("IP SET");
    ...
    GST_DEBUG ("Bitrate SET");

    guint desired_encode_speeed = (guint) encode_speed;
    data->encode_speed = desired_encode_speeed;

    GST_DEBUG ("Encode Speed SET");
...
static jboolean gst_native_class_init(JNIEnv *env, jclass klass) {
    ...
    if (!custom_data_field_id || !set_message_method_id || !on_gstreamer_initialized_method_id) {
         ...
         */
        __android_log_print(ANDROID_LOG_ERROR, "video_encoder",
...
static void gst_native_destination(JNIEnv *env, jobject thiz, jstring ip, jint port) {
    CustomData *data = GET_CUSTOM_DATA (env, thiz, custom_data_field_id);
    if (!data) return;
...
static JNINativeMethod native_methods[] = {
        {"nativeInit",           "(Ljava/lang/String;III)V", (void *) gst_native_init},
